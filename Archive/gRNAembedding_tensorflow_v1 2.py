...
if __name__ == "__main__":
    ...
    this_colors = 0*np.random.rand(len(X_transformed),9)
    print(this_labels)
    ...
    for x_ind in range(0,len(X_transformed)):
        ...
        if 0.10>=this_labels[x_ind]<0.20:
        ...
        if 0.50>=this_labels[x_ind]>0.60:
        ...
        if 0.70>=this_labels[x_ind]>0.80:
            this_colors[x_ind][6] = this_labels[x_ind]/np.max(this_labels)
        if 0.80>=this_labels[x_ind]>0.90:
            this_colors[x_ind][7] = this_labels[x_ind]/np.max(this_labels)
        if 0.90>=this_labels[x_ind]:
            this_colors[x_ind][8] = this_labels[x_ind]/np.max(this_labels)
